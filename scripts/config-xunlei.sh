...
if [ "$xunlei_basic_request" = "20" ]; then
	if [ -e $xunleiPath/portal ] && [ -x $xunleiPath/portal ]; then
		...
		dbus remove __event__onwanstart_xunlei
                dbus remove __event__onwanstart_xunlei_watch
	...
	if [ -e $xunleiPath/portal ] && [ -x $xunleiPath/portal ]; then
		...
		$xunleiPath/portal &
		dbus event onwanstart_xunlei /koolshare/scripts/start-xunlei.sh
                dbus event onwanstart_xunlei_watch /koolshare/scripts/watch-xunlei.sh
        else
		export xunlei_basic_status="00"
	fi
